...
/**
 * @file
 * Class to fetch workspaces.
 */

/**
 * Class TripolisWorkspaceService.
 */
class TripolisWorkspaceService extends TripolisAPISoap {
  /**
  ...
  public function __construct($client, $username, $password) {
    ...
    $this->setupSoap('WorkspaceService');
  }

  /**
   * Retrieve all workspaces based on database ID.
   *
   * @param string $type
   *   Group type.
   *
  ...
  public function getByContactDatabaseId($type = '') {
    if (!empty($this->dbId)) {
      try {
        ...
        if (isset($result['workspaces']['workspace'])) {
          return $result['workspaces']['workspace'];
        }
      }
      catch (SoapFault $e) {
        $this->throwError($e);
